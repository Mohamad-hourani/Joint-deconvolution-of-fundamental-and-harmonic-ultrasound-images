...
[SF,SH,r,z]=spectrum_of_cells_gliss(rf./max(rf(:)),110,1,fs);              % Calculation of the attenuation matrix by the ratio of the energy 
                                                                           %of the harmonic over the fundamental spectrum               
W=z;
Winv=1./z;
clear sgnFIG5;  
%%
...
H11=h_est1(n-40+1:n+40+1,n-10+1:n+10);
H22=h_est2(n-40+1:n+40+1,n-10+1:n+10+1);
H1=H11/max(H11(:));
H2=H22/max(H22(:));
...
[FB1,FBC1,F2B1,Bpad1] = Operator_PSF(H1,y1);                               % Operator of the estimated fundamental PSF
[FB2,FBC2,F2B2,Bpad2] = Operator_PSF(H2,y2);                               % Operator of the estimated harmonic PSF
% Normalized data
%% Presenting inputs
figure,
...
H=figure,
set(H,'position',pos)
subplot(151),imagesc(rf2bmode(y1,0.02)),colormap('gray'),title('Fundamental B-mode image')
subplot(152),imagesc(rf2bmode(y2,0.02)),colormap('gray'),  title( 'Harmonic B-mod image' )
subplot(155),imagesc(rf2bmode(x_joint,0.02)),colormap('gray'),title('Joint solution')
subplot(153),imagesc(rf2bmode(x_lasso_fund,0.02)),colormap('gray'),title('Lasso fundamental solution')
subplot(154),imagesc(rf2bmode(x_lasso_har,0.02)),colormap('gray'),  title( 'Lasso harmonic solution' )
